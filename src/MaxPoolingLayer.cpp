...
bool MaxPooling_Layer::generate(Tensor *activation, Tensor *pre_grads){
	...
	grads = new Tensor(x_size, y_size, activation->getZ());
	return true;
}
			...
			for (int x_pos = 0; x_pos < activation->getX(); x_pos+=x_receptive){
				...
				for (int y_step = 0; y_step < y_receptive; y_step++){
					for (int x_step = 0; x_step < x_receptive; x_step++){
						...
						pre_grads->getArray(z_pos, y_pos+y_step)[x_pos+x_step] = 0;
				...
				pre_grads->getArray(z_pos,y_pos+max_node_index_y)[x_pos+max_node_index_x]= grads->getArray(z_pos,y_pos/y_receptive)[x_pos/x_receptive];
			}
		}
	}
	return true;
